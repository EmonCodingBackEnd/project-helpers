...
import helper.archetype.share.common.DictDefinition;
import helper.archetype.share.common.JsonConverter;
import helper.archetype.share.common.cache.redis.AppRedisKey;
import helper.archetype.share.common.param.po.BaseParamPO;
...
public class ParamCache {
    ...
    public static <T extends BaseParamPO> T findParamNoisy(
        ...
        if (!optional.isPresent()) {
            ...
            throw new AppException(AppBaseStatus.systemExpectedError(), "根据参数代码找不到对应参数");
        }
        T t = optional.get();
        if (!t.isEnabled()) {
            log.error("【参数查询】参数已被禁用, paramCode={}", baseParam.getName());
            throw new AppException(AppBaseStatus.systemExpectedError(), "参数已被禁用");
        }
        return t;
    }
